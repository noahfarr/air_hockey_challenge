...
class ChickenHockeyAgent(AgentBase):
    ...
    def _bezier_to_cart_raw(self):
        if self.bezier_planner.t_final <= 0:
            if self.bezier_planner.p0 is not None:
                p0 = np.asarray(self.bezier_planner.p0).reshape(-1)
            else:
                p0 = np.zeros(2)
            zeros = np.zeros_like(p0)
            return p0[None, :], zeros[None, :], zeros[None, :]
